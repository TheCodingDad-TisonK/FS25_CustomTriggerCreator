<#
.SYNOPSIS
    Extracts CustomTriggerCreator-related errors and warnings from FS25 game log.

.DESCRIPTION
    Parses the Farming Simulator 25 log.txt file and extracts all entries
    related to the CustomTriggerCreator mod, categorizing them by severity and type.
...
Write-Header "CustomTriggerCreator Log Analyzer"
...
# Patterns to match
$patterns = @{
    # CustomTriggerCreator specific patterns
    CustomTriggerCreatorLine = 'CTC|FS25_CustomTriggerCreator'
    ModLoaded = 'v(\d+\.\d+\.\d+)'
    ...
    # Specific CustomTriggerCreator patterns
...
foreach ($i in 0..($lines.Count - 1)) {
    ...
    # Check if line is CustomTriggerCreator related
    if ($line -match $patterns.CustomTriggerCreatorLine) {
        $stats.CustomTriggerCreatorLines++
    ...
    if ($line -match $patterns.XmlParse -and $line -match 'CustomTriggerCreator') {
        $stats.XmlErrors += @{
            Line = $i + 1
            Text = $line.Trim()
            Type = "XmlError"
...
$report = @()
$report += "=" * 60
$report += "CustomTriggerCreator Error Report"
...
$report += "CustomTriggerCreator-Related Lines: $($stats.CustomTriggerCreatorLines)"
