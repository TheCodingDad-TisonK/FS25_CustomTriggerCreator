<#
.SYNOPSIS
    FS25_CustomTriggerCreator Mod Validator (PowerShell version)
...
Write-Header "FS25_CustomTriggerCreator Mod Validator"
...
foreach ($file in $LuaFiles) {
    ...
    for ($i = 0; $i -lt $lines.Count; $i++) {
        ...
        if (-not $isComment -and $line -match '\bprint\s*\(' -and $line -notmatch 'CustomTriggerCreator\.log' -and $RelPath -notmatch 'CustomTriggerCreatorCore\.lua') {
            Add-Warning -File $RelPath -Category "DEBUG_CODE" -Line $lineNum -Message "Raw print() - use CTC.log* instead"
        }
    }
}
...
# Only flag ctc_* keys as missing (base game keys are expected to be in game's i18n)
foreach ($file in $LuaFiles) {
    $RelPath = $file.FullName.Substring($ModPath.Length + 1)
    $content = Get-Content $file.FullName -Raw -ErrorAction SilentlyContinue
    if (-not $content) { continue }
    ...
    for ($i = 0; $i -lt $lines.Count; $i++) {
        ...
        foreach ($match in $keyMatches) {
            ...
            # Only flag ctc_* keys as errors (base game keys are assumed to exist)
            # Skip keys ending with _ (dynamic key construction like "ctc_fluid_" .. fluidType)
            if ($key.StartsWith('ctc_') -and -not $key.EndsWith('_') -and $key -notin $TransKeys) {
